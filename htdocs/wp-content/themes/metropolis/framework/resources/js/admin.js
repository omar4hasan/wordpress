...
function googleFont() {
	...
	fonts.each(function() {
		...
		weight.switcher(function(weight) { 
			preview.css({ 'font-weight': weight }); 
		});
		style.switcher(function(style) { 
...
function savepages()
{
	jQuery('.hb-section form:has(.ajax_save)').submit(function() {
		var form = jQuery(this);
		var button = form.find('.hb-save input[type="submit"]');
		button.attr("disabled", "disabled");

		var message_container = jQuery("#message-container");
	    var buttonVal = button.val();
	    button.val(button.attr('data-saving-text'));

		jQuery.post(form.attr("action"), form.serialize(), function(data){
	        button.val(buttonVal);

	        THB_Message.show(data.message, 'success');
 ...
 */
(function($) {
	$.fn.appear = function(speed, callback) {
		return this.each(function() {
		...
		execute: function( selector ) {
			...
			uploadFields.each(function() {
				...
				uploadButton.live('click', function() {
					...
					var old_sent_to_editor = window.send_to_editor;
