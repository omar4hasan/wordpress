...
if(!class_exists('AQ_Block')) {
	class AQ_Block {
	 	...
	 	function before_block($instance) {
	 		extract($instance);
	 		$column_class = $first ? 'first' : ''; // aq-first
	 		
	 		//echo '<div id="aq-block-'.$template_id.'-'.$number.'" class="aq-block aq-block-'.$id_base.' aq_'.$size.' '.$column_class.' clearfix">';
	 		$wp_autodiv = ( isset($wp_autodiv) ) ? $wp_autodiv : 0;
	 		
		 	if($wp_autodiv == 0):
				echo '<div id="shine-block-'.$template_id.'-'.$number.'" class="shine-block block-'.$id_base.' '.$size.' '.$column_class.' clearfix">';
			endif;
