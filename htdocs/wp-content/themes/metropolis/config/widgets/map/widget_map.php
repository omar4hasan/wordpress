...
function map_details($width, $height, $latlong='', $zoom=10, $address='', $mapTypeId='ROADMAP'){
	?>	
...
class WP_Widget_Map extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$mapTypeId = isset($instance['maptypeid']) ? $instance['maptypeid'] : 'ROADMAP';
		$address = isset($instance['address']) ? $instance['address'] : thb_get_option("_contact_address");

		echo $before_widget;
		if ( $title )
			echo $before_title . $title . $after_title;
			map_details($width, $height, $latlong, $zoom, $address, $mapTypeId);
		echo $after_widget;
	}
