...
class adminpage {
	...
	protected function form_nav($sections) {
		...
		$nav_tpl->assign('tabindex', isset($_POST['tab-index']) ? $_POST['tab-index'] : 1);
	...
	protected function form_open($index) {
		...
		$form_tpl->assign("action", site_url() .'/wp-admin/admin-ajax.php');
		// $form_tpl->assign("action", thbCurrentURL());
		// $form_tpl->assign('i', isset($_POST['tab-index']) ? $_POST['tab-index'] : 1);
...
class optionpage extends adminpage {
	...
	public function controls() {
		...
		$controls_tpl->assign("saving", __("Saving...", THEMENAME));
...
class duplicablepage extends adminpage {
	...
	public $classic_save = false;
	
	public function duplicablepage($fields, $classic_save=false) {
		global $wpdb;
		...
		$this->classic_save = $classic_save;
	...
	protected function form_open($index) {
		...
		$action = $this->classic_save ? thbCurrentURL() : site_url() .'/wp-admin/admin-ajax.php';

		$form_tpl->assign("action", $action);
		return $form_tpl->draw('form_open', true);	
	}
	...
	public function controls($position) {
		...
		$controls_tpl->assign("field_type", $this->field_type);
		$controls_tpl->assign("submit_class", !$this->classic_save ? 'ajax_save' : '');
 ...
 * @return void
 */
function thb_verify_nonce($nonce) {
    if(!isset($_POST['thb_noncename']) || !wp_verify_nonce($_POST['thb_noncename'], plugin_basename($nonce))){
        $response['error'] = true;
        $response['message'] = __('You do not have sufficient permissions to save these options.', THEMENAME );
...
/**
 * Save a duplicable page.
 * @return void
 */
function thb_duplicable_save() {
	$response['error'] = false;
	$response['message'] = '';
	$response['type'] = '';

    thb_verify_nonce('thb_duplicable_options');
	...
	$data = optionpage::clearPostData($_POST);

	$wpdb->query("DELETE FROM {$table_name} WHERE type = '{$type}'");

	$fields = array_keys($data);

	if(!empty($fields) && isset($fields[0]) && isset($data[$fields[0]])) {
		$num_items = count($data[$fields[0]]);
		$items_keys = array_keys($data[$fields[0]]);
		...
		foreach($items_keys as $i) {
			$item = array();
			foreach($fields as $field) {
				if( !in_array($field, array('action', 'thb_noncename')) ) {
					$tokens = explode("_", $field);
					$key = $tokens[count($tokens)-1];
					$item[$key] = $_POST[$field][$i];
				}			
			}
			$slug = thb_make_slug($item['title']);

			$item['type'] = $type;
			if( empty($item['name']) ) {
				$item['name'] = $slug;
				// $item['name'] = $slug . md5($item['title'] . time() . $j);
			}
	...
	$response['message'] = __( 'All saved', THEMENAME );    
	echo json_encode($response);
	die;
}
add_action('wp_ajax_thb_duplicable_save', 'thb_duplicable_save');

/**
 * Save an options page.
 * @return void
 */
function thb_framework_save(){
    $response['error'] = false;
    $response['message'] = '';
    $response['type'] = '';
    
    thb_verify_nonce('thb_framework_options');
...
add_action('wp_ajax_thb_framework_save', 'thb_framework_save');
